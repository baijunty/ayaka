          ...
          ListTile(
            ...
            subtitle: Text(
              Iterable.iterableToFullString(
                _settingsController.config.languages.map(
                  (l) => mapLangugeType(context, l),
                ),
                '',
                '',
              ),
            ...
            onTap: () async {
              ...
              for (var item in languages) {
                item['selected'] = _settingsController.config.languages
                    .contains(item['name']);
              }
              if (mounted) {
          ...
          ListTile(
            ...
            onTap: () async {
              var items = await _settingsController
                  .hitomi(type: HitomiType.Local)
                  .translate(
                    _settingsController.config.excludes.fold(
                      <Label>[],
                      (acc, l) =>
                    ...
                    _settingsController.updateConfig(
                      _settingsController.config.copyWith(
                        excludes: data
                            .map(
                              (e) => [
                                FilterLabel(type: e['type'], name: e['name']),
                              ],
                            )
                            .toList(),
                      ),
...
class MultiSelectListView extends StatefulWidget {
  const MultiSelectListView({super.key, required this.items});
        ...
        actions: [
          IconButton(
            onPressed: () => Navigator.pop(
              context,
              widget.items.where((item) => item['selected'] ?? false).toList(),
            ),

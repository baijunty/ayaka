...
class _AnimatedSaverDialogView extends State<AnimatedSaverDialog> {
  ...
  Future<Uint8List> buildAnimatedImage(
    ...
    var data = await context.read<GalleryManager>().makeAnimatedImage(
      ...
      size: size,
      onProgress: (index, total) => chunkEvents.add(
        ImageChunkEvent(
          cumulativeBytesLoaded: index,
        ...
        child: ThumbImageView(
          ProxyNetworkImage(
            ...
            key: widget.selected.fold(
              ...
              (previousValue, element) => previousValue..write(element.name),
            ),
          ),
          aspectRatio:
              widget.selected.first.width / widget.selected.first.height,
        ),
      ),

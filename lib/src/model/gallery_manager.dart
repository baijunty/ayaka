...
class GalleryManager with ChangeNotifier {
  ...
  Future<void> addTask(String command) async {
    controller.remoteLib
        ? await controller.manager.dio.post(
            ...
            options: Options(headers: {'x-real-ip': await localIp()}),
          )
  ...
  Future<Map<String, dynamic>> checkExist(List<dynamic> ids) async {
    return controller.remoteLib
        ? controller.manager.dio
              .post<String>(
                '${controller.config.remoteHttp}/checkId',
  ...
  Future<Uint8List> makeAnimatedImage(
    ...
    return images
        ...
        .asyncMap((event) {
          ...
          return c.getImageThread().then(
            (value) => value?..frameDuration = 200,
          );
        })
        .filterNonNull()
        .reduce(
          (previous, element) => element.frames.fold(
            previous,
            (previousValue, element) => previous..addFrame(element),
          ),
        )
        .then((value) {
